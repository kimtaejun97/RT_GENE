...
if __name__ == "__main__":
    ...
    hdf_file = h5py.File(os.path.abspath(os.path.join(args.rt_gene_root, 'rtgene_dataset_LR.hdf5')), mode='w')
    for subject_id, subject_data in enumerate(subject_path):
        ...
        # with open(os.path.join(subject_data, "label_combined_test.txt"), "r") as f:
            ...
            for line in tqdm(_lines, desc="Subject {}".format(subject_id)):
                if not header:
                    ...
                    if subject_id =="s000":
                        ...
                        left_img_path = os.path.join(subject_data, "inpainted/left_LR/", subject_id+"_left_{:0=6d}_rgbx4.png".format(int(split[0])))
                        right_img_path = os.path.join(subject_data, "inpainted/right_LR/", subject_id+"_right_{:0=6d}_rgbx4.png".format(int(split[0])))
                    if os.path.exists(left_img_path) and os.path.exists(right_img_path):
                        ...
                        head_phi = float(split[1].strip())
                        head_theta = float(split[2].strip())
                        gaze_phi = float(split[3].strip())
                        gaze_theta = float(split[4].strip())
