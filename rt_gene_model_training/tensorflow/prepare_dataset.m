...
load_path = '../rt_gene_dataset/';
save_path = '../mtcnn_twoeyes_inpainted_eccv/';
...
for person_idx = 4:length(person_load_path_name)
    ...
    %%
    [data_num, fieldnum] = size(combined_file.data);
    ...
    for i=1:data_num
        file_idx(i) =combined_file.data(i,1);
        head_pose(i,1) = combined_file.data(i,2);
        head_pose(i,2) = combined_file.data(i,3);
        gaze(i,1) = combined_file.data(i,4);
        gaze(i,2) = combined_file.data(i,5);
    end
    ...
    save(train_savename, 'train', '-v6');
    test_savename = person_save_path_name_test{person_idx};
    save(test_savename, 'test', '-v6');
